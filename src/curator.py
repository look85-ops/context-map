...
ПРАВИЛА ЦИТИРОВАНИЯ (НАРУШЕНИЕ НЕДОПУСТИМО):
Используй ТОЛЬКО URL из списка "ИСТОЧНИКИ ДЛЯ ЦИТИРОВАНИЯ" в конце новостей.

Формат: [Короткое название](URL)
Пример: [РБК](https://www.rbc.ru/economics/...) а не [1] и не bare URL

В конце дайджеста — блок "Источники:" со списком использованных ссылок.
